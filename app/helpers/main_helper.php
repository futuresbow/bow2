...
function ws_frontendView($file, $adat = null, $kimenet = false) {
	...
	$eleres = FCPATH.'public/'.TEMAMAPPA.'/'.FRONTENDTEMA.'/'.$filenev;
...
function ws_autoload($modul) {
	$file = FCPATH.'mods/'.$modul.'/autoload.php';
	if(file_exists($file)) {
		include_once($file);
		return true;
	} else {
...
function ws_hookFuttatas($belepesipont, $param) {
	...
	foreach($hookBelepesiPontok[$belepesipont] as $eleres) {
		...
		$file = FCPATH.'mods/'.$modul.'/'.$osztaly.'.php';
		if(file_exists($file)) {
			include_once($file);
			$o = new $osztaly() ;
			$o->{$metodus}($param);
...
function modulView($modul,$file, $data = array()) {
	$ci = getCI();
	$modulViewFile = FCPATH.'mods/'.$modul.'/view/'.$modul.'_'.$file.'.php'; 
...
function ws_temaadatok() {
	$temak = scandir(FCPATH.'public/'.TEMAMAPPA);
	if($temak) {
		foreach($temak as $tema) {
			if($tema == '.' or $tema == '..') continue;
				
			$temaKonyvtar = FCPATH.'public/'.TEMAMAPPA.'/'.$tema.'/';
...
function ws_moduladatok() {
	...
	if($modulok) {
		foreach($modulok as $modul) {
			...
			$modulKonyvtar = FCPATH.'mods/'.$modul.'/';
...
function widget($utvonal, $param = false) {
	...
	$ut = FCPATH.'mods/'.$modul.'/'.$osztaly.'.php';
	if(!file_exists($ut)) return false;
	include_once(FCPATH.'mods/'.$modul.'/'.$osztaly.'.php');
	if(!class_exists($osztaly)) return false;
	$o = new $osztaly;
	
	if(!method_exists($o, $metodus)) return "Widget nem található: ".'mods/'.$modul.'/'.$osztaly.' / '.$metodus;
