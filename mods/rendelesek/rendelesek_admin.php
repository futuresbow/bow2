...
class Rendelesek_admin extends MY_Modul {
	...
	public function lista () {		globalisMemoria("Nyitott menüpont",'Rendelések');
		...
		foreach($lista as $sor) {
			$sor->statuszvaltoztatas = '<div class="input-group"><select class="form-control" style="width:130px;display: inline-block;"><option value="0">Válassz</option>'.$statuszopciok.'</select><div class="input-group-append"><button class="btn btn-outline-success " onclick="window.location.href=\''.ADMINURL.'rendelesek/statszbeallitas/'.$sor->id.'/\'+$(this).prev().val()">OK</button></div></div><b>Státusz: '.$statuszArr[$sor->statusz].'</b>';
			$vevoRs = $this->Sql->sqlSor("SELECT * FROM ".DBP."rendeles_felhasznalok WHERE id = {$sor->rendeles_felhasznalo_id} LIMIT 1");
			$sor->vevo = $vevoRs->vezeteknev.' '.$vevoRs->keresztnev;
			$sor->osszar = ws_arformatum($sor->osszar).' Ft';
			$sor->ido = date('Y-m-d', strtotime($sor->ido));
	...
	public function megrendelesStatuszvaltozasHook($param) {
		...
		switch($kod) {
			...
			case 'LEA':
				...
				include(FCPATH.'mods/hirlevel/autoload.php');
	...
	public function statszbeallitas() {
		...
		// termékek bootstrap
		include_once(FCPATH.'mods/termek/autoload.php');
