...
class Oldalak_admin extends MY_Modul {
	...
	public function tartalomszerkesztes() {		globalisMemoria("Nyitott menüpont",'Oldalak');
		...
		if($this->input->post('a')) {			// címet kellene szerezni a bejegyzéshez:
			...
			if(!file_exists(FCPATH.'mods/'.$modul.'/'.$osztaly.'.php')) {
		...
		// akkor a mods/slider/slider.php index metódusát futtatja a frontend
		// és a mods/slieder/slider_admin.php sliderindex_beallito metódusa lesz a beállító
		//
		...
		if(!file_exists(FCPATH.'mods/'.$modul.'/'.$osztaly.'.php')) {
			...
			//print $osztaly.'  osztály:' .FCPATH.'mods/'.$modul.'/'.$osztaly.'.php'."\n";
